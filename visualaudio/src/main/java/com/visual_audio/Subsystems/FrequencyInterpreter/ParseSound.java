...
public class ParseSound {
    ...
    // Use the Fast Fourier Transform algorithim to get the frequencies of the audio
    // data to parse hz from low to high
    public void getFrequencies(double[] audioData) {
        ...
        double[] magnitude = magnitude(fourierResult);

        this.setAverageHighHz(magnitude, audioData);
        this.setAverageLowHz(magnitude, audioData);
    }

    private int bitShift(double[] audioData) {
        int fourierSize = 1;
    ...
    private double[] hannWindow(double[] audioData, int fourierSize) {
    ...
    private double[] fourierTransform(double[] audioInput) {
    ...
    private void cooleyTukey(double[] complexAudio, int n) {
        if (n == 1) {
    ...
    private double[] magnitude(double[] results) {
